    ...
    if abs(r) < 0.925:
        ...
        bvn = asr * np.sum(
            np.multiply(
                ...
                np.exp(
                    np.divide(
                        np.multiply(dim1sn1, hkdim2) - hsdim2,
            ...
            + np.multiply(
                ...
                np.exp(
                    np.divide(
                        np.multiply(dim1sn2, hkdim2) - hsdim2,
                        (1 - np.outer(dim1, sn22)),
                    )
                ),
            ),
            axis=1,
        ) / (4 * np.pi) + np.multiply(norm_cdf(-dh), norm_cdf(-dk))
    else:
        if r < 0:
            dk = -dk
            hk = -hk
        ...
        if abs(r) < 1:
            ...
            bvn[ind] = bvn[ind] - np.multiply(
                np.multiply(
                    np.multiply(np.exp(-hk[ind] / 2.0), ncdfxmyt[ind]), xmy[ind]
                ),
                1.0
                - np.multiply(
            ...
            for ix in [-1, 1]:
                ...
                sp1 = 1.0 + np.multiply(
                    np.multiply(cdim2, dim1xs), 1.0 + np.multiply(rhk16dim2, dim1xs)
                )
                ep1 = np.divide(
                    np.exp(
                        np.divide(
                            -np.multiply(hkdim2, (1.0 - dim1rs)), 2.0 * (1.0 + dim1rs)
                        )
                    ),
                    dim1rs,
                )
                bvn = bvn + np.sum(
                    np.multiply(
                        np.multiply(
                            np.multiply(sopmr, dim1w), np.exp(np.multiply(asr1, ind1))
                        ),
